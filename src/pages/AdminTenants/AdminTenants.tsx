...
import { AdminTenantsEntry, type Tenant } from "./index";

export function AdminTenants() {
  const queryClient = useQueryClient();

  const { getTenants, deleteTenant } = useProvider();
  ...
  const columns: Column<Tenant>[] = [
    ...
    },
    {
      accessorKey: "phone",
      header: "Phone",
      cell: (item) => <span>{item.phone || "-"}</span>,
    },
    {
      accessorKey: "address",
      header: "Address",
      cell: (item) => <span>{item.address || "-"}</span>,
    },
    {
      accessorKey: "domain",
      header: "Domain",
      cell: (item) => <span>{item.domain || "-"}</span>,
    },
    {
      accessorKey: "businessType",
      header: "Business Type",
      cell: (item) => <span>{item.businessType || "-"}</span>,
    },
    {
      accessorKey: "id",
      header: "Actions",
