...
export function BrandsEntry({ isOpen, onClose, brand, onSuccess }: FormProps) {
  const queryClient = useQueryClient();

  const { createNewBrand, updateBrand } = useProvider();
  ...
  // Update local state when brand prop changes
  useEffect(() => {
    setFormData(brand || {});
    setErrors({});
  }, [brand, isOpen]);
  ...
  const createMutation = useMutation({
    mutationFn: (data: unknown) => createNewBrand(data),
    onSuccess: (data: any) => {
