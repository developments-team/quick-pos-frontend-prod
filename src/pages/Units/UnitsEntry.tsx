...
export function UnitsEntry({ isOpen, onClose, unit, onSuccess }: FormProps) {
  const queryClient = useQueryClient();

  const { createNewUnit, updateUnit } = useProvider();
  ...
  // Update local state when unit prop changes
  useEffect(() => {
    setFormData(unit || {});
    setErrors({});
  }, [unit, isOpen]);
  ...
  const createMutation = useMutation({
    mutationFn: (data: unknown) => createNewUnit(data),
    onSuccess: (data: any) => {
